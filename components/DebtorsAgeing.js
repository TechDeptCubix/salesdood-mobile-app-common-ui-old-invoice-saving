...
const DebtorsAgeing = () => {
    ...
    // const colors = debtorsData.length > 0 && data.datasets[0].data.map((_, index) => hexValues[index % hexValues.length]);
    ...
    // const dataObject = debtorsData && debtorsData[0];
    // const labels = [];
    // const dataPoints = [];
    ...
    const fetchDebtorsData = async () => {
        setShowLoader(true)
        try {
            const response = await fetch(`${DashUrl}?cmpcode=${cmpcode}&guid=${privatek}&mod=DEBTORS_AGEING&s1=%27%27&s2=%27%27&s3=%27%27&i1=0&i2=0&dt1=11-DEC-2023&dt2=11-DEC-2023`);
            ...
            if (data && data.length > 0) {
                ...
                for (const key in firstEntry) {
                    if (key !== "AMOUNT") {
                        newLabels.push(key);
                        newDataPoints.push(firstEntry[key]);
                    }
                }
    ...
    return (
        <View style={styles.CashWrapper}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Text style={{ fontSize: 18 }}>Debtors Ageing</Text>
            ...
            <View style={{ marginTop: 12, marginBottom: 12 }}>
                ...
                {
                    !showLoader && debtorsData.length === 0 &&
                    <View>
                        <Text style={{
                            color: 'red'
                ...
                <ScrollView horizontal={true}>
                    {
                        debtorsData && debtorsData.length > 0 &&
