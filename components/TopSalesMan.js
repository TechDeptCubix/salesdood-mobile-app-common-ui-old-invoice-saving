    ...
    useEffect(() => {
        ...
        if (cmpcode && privatek) {
            if (selectedDateTab === 'Day') {
                ...
                const url = `${DashUrl}?cmpcode=${cmpcode}&guid=${privatek}&mod=T_SALESPERSON&s1=%27%27&s2=%27%27&s3=%27%27&i1=100&i2=0&dt1=${formattedDay}&dt2=${formattedDay}`;
                axios.get(url)
                    .then(response => {
                        const data = response.data;
                        // setDeptSalesData(data);

                        const sortedItems = data.sort((a, b) => b.SALES_AMT - a.SALES_AMT);
                    ...
                    .catch(error => console.error('DayToggelErr:', error));
            }
            ...
            if (selectedDateTab === 'Month') {
                ...
                const url = `${DashUrl}?cmpcode=${cmpcode}&guid=${privatek}&mod=T_SALESPERSON&s1=%27%27&s2=%27%27&s3=%27%27&i1=100&i2=0&dt1=${formatFirstdDay}&dt2=${formatLastDay}`;
                ...
                axios.get(url)
                    ...
                    .catch(error => console.error('MonthToggelErr:', error));
            }
            ...
            if (selectedDateTab === 'Year') {
                ...
                axios.get(url)
                    ...
                    .catch(error => console.error('YearToggelErr:', error));
            }
        }
    ...
    return (
        <ImageBackground style={styles.CashWrapper}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Text style={{ fontSize: 18 }}>Top Sales Man</Text>
            ...
            <ScrollView nestedScrollEnabled={true} contentContainerStyle={{ width: '100%', alignItems: 'center' }}>
                <ScrollView horizontal={true}>
                    <View style={styles.TableContainer}>
...

                        {
                            topSalesPerson && topSalesPerson.length > 0 && topSalesPerson.map((item, index) => (
                                <View style={styles.tableRow} key={index}>
                                    <Text style={styles.dataCell}>{item.SmanName ? item.SmanName.trim() : "Nil"}</Text>
                                    <Text style={styles.dataCell}>{item.SALES_AMT ? item.SALES_AMT : "Nil"}</Text>
                                </View>
                        ...
                        {
                            topSalesPerson && topSalesPerson.length === 0 &&
                            <View>
                                <Text style={{
                                    color: 'red'
                        ...
                        {/* <View style={styles.tableRow}>
                            <Text style={styles.dataCell}>AJMAL</Text>
