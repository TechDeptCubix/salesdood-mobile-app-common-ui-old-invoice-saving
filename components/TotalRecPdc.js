...
const TotalRecPdc = ({ setShowTotalRecPop }) => {
    ...
    useEffect(() => {
        ...
        if (cmpcode && privatek) {
            ...
            fetch(url)
                ...
                .catch(error => {
                    console.error('RxcPDCError:', error)
                    setShowLoader(false)
                });
        }
    }, [date, cmpcode, privatek])

    const totalAmount = receivedPdcData && receivedPdcData.reduce((acc, item) => acc + item.Amount, 0);
    ...
    return (
        <ImageBackground style={styles.BankWrapper}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Image source={require('../dashImages/TotalRec.png')} />
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 16 }}>
                <Text style={{ fontSize: 18 }}>Total Received PDC</Text>
                <TouchableOpacity onPress={() => setShowTotalRecPop(true)}>
