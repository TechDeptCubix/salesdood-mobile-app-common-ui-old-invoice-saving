...
const TopGroups = () => {
    ...
    useEffect(() => {
        if (grpData) {
            const data = grpData.map(item => item.GRP); // Extract group names
    ...
    useEffect(() => {
        ...
        if (cmpcode && privatek) {
            ...
            if (selectedDateTab === 'Day') {
                ...
                const url = `${DashUrl}?cmpcode=${cmpcode}&guid=${privatek}&mod=TOP_GROUP&s1=%27%27&s2=%27%27&s3=%27%27&i1=100&i2=0&dt1=${formattedDay}&dt2=${formattedDay}`;
                axios.get(url)
                    .then(response => {
            ...
            if (selectedDateTab === 'Month') {
                ...
                const url = `${DashUrl}?cmpcode=${cmpcode}&guid=${privatek}&mod=TOP_GROUP&s1=%27%27&s2=%27%27&s3=%27%27&i1=100&i2=0&dt1=${formatFirstdDay}&dt2=${formatLastDay}`;
    ...
    return (
        <View style={styles.CashWrapper}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Text style={{ fontSize: 18 }}>Top 5 Groups</Text>
