...
const Receipt = ({navigation}) => {
  ...
  const [isCallingListOfAccountsApi, setCallingListOfAccountsApi] =
    useState(false);
...
`;
  ...
  const fetchTop50Customers = async () => {
    ...
    try {
      ...
      if (cmpcode?.trim().toUpperCase() == 'SOCA') {
        ...
        console.log(
          'filteredArrayBasedOnSalesman>> Receipt new',
          filteredArrayBasedOnSalesman,
          salesName.trim().toUpperCase(),
        );
  ...
  const fetchStatementData = async () => {
    setShowLoader(true);
    console.log(
      `${appUrl}OutstandingStmt/${cmpcode}/OUT_ACC1/${accountNo}/-/${fromDate}/${toDate}/-`,
    );
    try {
  ...
  const handlePress = (inv, balance) => {
    ...
    setSelectedInv(prevSelectedInv => {
      if (prevSelectedInv.includes(inv)) {
        ...
        let newSelectedInvoices = selectedtInvTransactions?.filter(
          item => item.invoice_number != inv,
        );
  ...
  const changeValuesInInputAmount = (
    invoiceNumber,
    itemAmount,
    itembalance,
    value,
    ...
    console.log(
      'invoiceNumber,itemAmount,itembalance,  value, inputName>>',
      invoiceNumber,
      itemAmount,
      itembalance,
      value,
                  ...
                  <View style={styles.StockListItem} key={index}>
                    ...
                    {expandedItems.includes(item.account) && (
                      <View style={styles.QtyAvlQtyCont}>
                        ...
                        <TouchableOpacity
                          style={[styles.QtyCont, {backgroundColor: '#D8D8DA'}]}
                  ...
                  <View style={{padding: 10}}>
                    ...
                    <View style={{position: 'relative'}}>
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          getListOfAccounts();
                          setShowListofAccounts(prev => !prev);
                        }}>
                        <Text style={{color: '#ffffff'}}>Select Account</Text>
                        <Image
                          style={{marginLeft: 10, width: 20, height: 20}}
                          source={ic_arrow_down}
                        />
                        {isCallingListOfAccountsApi && (
                      ...
                      <View style={{padding: 2}}>
                        <Text>{currentSelectedAccount?.Account}</Text>
                        <Text>{currentSelectedAccount?.Accdesc}</Text>
                      </View>
                    </View>
                    ...
                    <View>
                      <TextInput
                        ...
                        onChangeText={text => changeValuesInInput(text, 'bank')}
                        style={[
                          styles.TANDCInpCont,
                          {color: '#000000', width: '100%'},
                        ]}>
                        {currentCDetails.bank}
                      </TextInput>
                      ...
                      <TextInput
                        ...
                        onChangeText={text =>
                          changeValuesInInput(text, 'chqno')
                        }
                        style={[
                          styles.TANDCInpCont,
                          {color: '#000000', width: '100%'},
                        ]}>
                        {currentCDetails.chqno}
                      </TextInput>
                                            ...
                                            {selectedtInv.includes(
                                              ...
                                              <View style={{width: '100%'}}>
                                                {console.log(
                                                  ...
                                                  selectedtInvTransactions,
                                                  'value',
                                                  selectedtInvTransactions?.filter(
                                                    itemTr =>
                                                      itemTr.invoice_number ==
                                                      item.INV,
                                                  ),
                                                )}
                                                {selectedtInvTransactions
                                                  ...
                                                  .map(itemRes => {
                                                    return (
                                                      <View
                                                        ...
                                                        <TextInput
                                                          value={itemRes.amount}
                                                          placeholder="amount"
                                                          keyboardType="numeric"
                                                          onChangeText={text =>
                                                            changeValuesInInputAmount(
                                                              itemRes.invoice_number,
                                                              itemRes.amount,
