import { View, Text, StyleSheet, Dimensions, TouchableOpacity, ScrollView, FlatList, ActivityIndicator, Image } from 'react-native'
...
const QuotationList = () => {
    ...
    const fetchPreviousOrders = async () => {
        try {
            ...
            console.log("previous quotation api url ======+++", `${appUrl}proposal/${cmpcode}/quoteprevious/${salesMan}/${deptno}`)

            const response = await axios.get(`${appUrl}proposal/${cmpcode}/quoteprevious/${salesMan}/${deptno}`);
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <ScrollView style={styles.PreviousOrderWrap}>
                            ...
                            <TouchableOpacity style={styles.StockListItem} onPress={() => navigation.navigate('QuotationDetails', { orderId: item.so_no })}>
                                ...
                                <View style={styles.CustomerListCont}>
                                    ...
                                    <View style={styles.CustomerListMid}>
                                        <View style={{
                                            ...
                                            <Text style={[styles.StockListDescText, { width: '75%' }]}>{item.accdesc}</Text>
                                            <Text style={[styles.StockListDescTextSmall, { color: '#30B3A4', fontFamily: 'Lexend-Regular', }]}>{item.so_amount}</Text>
                                        </View>
                                        <View style={{
                                            flexDirection: 'row',
                                            width: '100%',
                                            ...
                                            <Text style={styles.StockListDescTextSmall}>{item.so_no}</Text>
                                            <View style={{
                                                marginLeft: 24,
                                                flexDirection: 'row'
                                            }}>
                                                {/* <Text style={[styles.StockListDescTextSmall,]}>Inv Date:</Text> */}
                                                <Text style={[styles.StockListDescTextSmall,]}>{formattedDate(item.so_date)}</Text>
                                            </View>

                                            {/* <TouchableOpacity style={[styles.PlusMinusCont, { marginLeft: 'auto' }]} onPress={() => toggleExpand(item.so_no)}>
                                                {
