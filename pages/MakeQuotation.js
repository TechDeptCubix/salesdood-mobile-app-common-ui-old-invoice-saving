import { View, Text, StyleSheet, Dimensions, Image, TouchableOpacity, TextInput, ScrollView, ActivityIndicator, KeyboardAvoidingView, Platform, Alert, Keyboard } from 'react-native'
...
const MakeQuotation = ({ route }) => {
    ...
    useEffect(() => {
        if (selectedCustomer) {
            ...
            if (selectedCustomer.CREDITMETHOD == 'CREDIT BLOCK' && selectedRadio === 'CREDIT') {
    ...
    const isDueDateExceeded = async () => {
        ...
        return new Promise((resolve, reject) => {
            ...
            axios.get(`${appUrl}MasterCount/${cmpcode}/duedays/${selectedCustomer.account?.trim()}/${deptNo} `).then((res) => {
    ...
    const NextClick = async () => {
        ...
        if (selectedCustomer || cashCustomerName) {
            ...
            if (selectedRadio === 'CREDIT' && selectedCustomer.CREDITMETHOD == 'CREDIT BLOCK') {
    ...
    const SaveItem = async () => {
        ...
        if (selectedStock && quantity && unitPrice && selectedCustomer && !cashCustomerName && selectedCustomer.CREDITMETHOD == 'CREDIT BLOCK') {
            ...
            if ((totalIncludingVAT && (totalIncludingVAT > selectedCustomer.Avai_Bal)) && selectedRadio === 'CREDIT') {
                Alert.alert(
                    'Amount Exceed',
                    'Amount exceed available limit. Please choose cash customer to add more items.',
                    [{ text: 'OK' }]
            ...
            console.log("else after itemexist ", selectedStock, "q", quantity, "u", unitPrice, "sc", selectedCustomer, "cn", cashCustomerName, "sc credit", selectedCustomer.CREDITMETHOD)
        }
        ...
        if (selectedStock && quantity && unitPrice && selectedCustomer && selectedCustomer.CREDITMETHOD == 'OPEN') {
            ...
            console.log("else after itemexist creditblock open ", selectedStock, "q", quantity, "u", unitPrice, "sc", selectedCustomer, "cn", cashCustomerName, "sc credit", selectedCustomer.CREDITMETHOD)
        }
    ...
    const logAsyncData = async () => {
        ...
        try {
            ...
            const storedSelectedCustomer = await AsyncStorage.getItem('selectedCustomer');
            ...
            const storedCashCustomerName = await AsyncStorage.getItem('cashCustomerName');
            const storedCashCustomerAddress = await AsyncStorage.getItem('cashCustomerAddress');
            const storedCashCustomerPhone = await AsyncStorage.getItem('cashCustomerPhone');
    ...
    useEffect(() => {
        if (selectedUserType === 'unreg') {
            ...
            if (selectedCustomer) {
                ...
                setCashCustomerPhone(selectedCustomer.phone && selectedCustomer.phone.trim())
            }
    ...
    useEffect(() => {
        ...
        if (selectedRadio === 'CREDIT' && selectedCustomer && selectedCustomer.CREDITMETHOD === 'CREDIT BLOCK' && selectedCustomer.Avai_Bal === 0) {
            setBlockNextButtonView(true)
            Alert.alert(
                'Amount Equal',
        ...
        if (selectedRadio === 'CREDIT' && selectedCustomer && selectedCustomer.CREDITMETHOD === 'CREDIT BLOCK' && selectedCustomer.Avai_Bal > selectedCustomer.Credit_Limit) {
            setBlockNextButtonView(true)
            Alert.alert(
                'Amount Exceed',
                'Available balance is greater than Credit limit. Please choose cash customer to add more items.',
    ...
    const checkIfitemStockisZero = async (item) => {
        ...
        if (cmpcode?.trim().toUpperCase() == "AUTOMAX") {
            ...
            if (resultOfPriceManagement) {
                Alert.alert("Price Management", "This item is required to update the new price. Please contact the concerned team. Selection Blocked")
                return
            } else {
                setSelectedStock(item)
            }
            ...
            <View style={styles.HomeWrap}>
                ...
                <KeyboardAvoidingView
                    ...
                    <View style={{ width: '100%', height: '100%' }}>
                        ...
                        <ScrollView contentContainerStyle={styles.MainScroll} bounces={false} keyboardShouldPersistTaps="handled">
                                        ...
                                        <>
...

                                            {
                                                ...
                                                <>
                                                    ...
                                                    <ScrollView contentContainerStyle={[styles.CheckStockListView]} keyboardShouldPersistTaps="handled">

                                                        {
                                                            customerData && customerData.length > 0 && customerData.map((item, index) => (
                                                                <TouchableOpacity style={styles.StockListItem} key={index} onPress={() => {
                                                                    Keyboard.dismiss();
                                                                        ...
                                                                        <View style={styles.CustomerListMid}>
                                                                            ...
                                                                            <View style={{
                                                                                ...
                                                                                <View style={{
                                                                                    ...
                                                                                    <Text style={[styles.StockListDescTextSmall, { fontSize: 11 }]}>{item.CREDITMETHOD}</Text>
                                                                                </View>
                                            ...
                                            <>
                                                {
                                                    ...
                                                    (!showCashCust) ?
                                                        <>

                                                            {
                                                                ...
                                                                <View style={styles.SelectedCustomerNewBanner}>
                                                                    <View>
                                                                        <Text style={styles.SelectCustNewText}>{selectedCustomer && selectedCustomer.Custname?.trim() !== '' ? selectedCustomer.Custname : ''}</Text>
                                                                    </View>
                                                                    <View style={styles.SelectCustNewBottomWrap}>
                                                                        <View style={styles.SelectCustBottomItem}>
                                                                            <Text style={styles.SelectCustNewBottomText}>Account :</Text>
                                                                            <Text style={[styles.SelectCustNewBottomText, { marginLeft: 8 }]}>{selectedCustomer && selectedCustomer.account?.trim() !== '' ? selectedCustomer.account : ''}</Text>
                                                                        </View>
                                                                        <View style={styles.SelectCustBottomItem}>
                                                                            <Text style={styles.SelectCustNewBottomText}>Status :</Text>
                                                                            <Text style={[styles.SelectCustNewBottomText, { marginLeft: 8 }]}>{selectedCustomer && selectedCustomer.CREDITMETHOD}</Text>
                                                                        </View>
                                                                    </View>
                                                                </View>
                                                            }
...

                                                            {
                                                                ...
                                                                <View style={styles.AddressDetailsWrap}>
                                                                    <View style={styles.AddressTopBanner}>
                                                                        <Text style={styles.CustHeadText}>Address Details</Text>
                                                                    </View>
                                                                    <View>
                                                                        <Text style={styles.CustHeadText}>{selectedCustomer && selectedCustomer.address1}</Text>
                                                                    </View>
                                                                    <View>
                                                                        <Text style={styles.CustHeadText}>{selectedCustomer && selectedCustomer.address2}</Text>
                                                                    </View>
                                                                    <View>
                                                                        <Text style={styles.CustHeadText}>{selectedCustomer && selectedCustomer.address3}</Text>
                                                                    </View>
                                                                </View>
                                                            }
                                                            ...
                                                            <View style={styles.AddressDetailsWrap}>
                                                                <View style={styles.AddressTopBanner}>
                                                                    <Text style={styles.CustHeadText}>Terms & Condition</Text>
                                                                </View>
                                                                ...
                                                                <View style={styles.TermsCondtitonInpWrap}>
                                                                    ...
                                                                    <View style={styles.TANDCInpItem}>
                                                                        <Text style={[styles.CustHeadText, { width: '20%' }]}>Payment</Text>
                                                                    ...
                                                                    <View style={styles.TANDCInpItem}>
                                                                        <Text style={[styles.CustHeadText, { width: '20%' }]}>Delivery</Text>
                                                                        <View style={styles.TANDCInpCont}>
                                                                            <TextInput
                                                                                style={styles.NewInputStyle}
                                                                    ...
                                                                    <View style={styles.TANDCInpItem}>
                                                                        <Text style={[styles.CustHeadText, { width: '20%' }]}>Validity</Text>
                                                                        <View style={styles.TANDCInpCont}>
                                                                            <TextInput
                                                                                style={styles.NewInputStyle}
                                                                    ...
                                                                    <View style={styles.TANDCInpItem}>
                                                                        <Text style={[styles.CustHeadText, { width: '20%' }]}>TRN</Text>
                                                                        <View style={styles.TANDCInpCont}>
                                                                            <TextInput
                                                                                style={styles.NewInputStyle}
                                                        ...
                                                        <>
                                                            ...
                                                            <View style={styles.AddressDetailsWrap}>
                                                                <View style={styles.AddressTopBanner}>
                                                                    <Text style={styles.CustHeadText}>Walkin Customer Details</Text>
                                                                </View>
                                                                ...
                                                                <View style={styles.TermsCondtitonInpWrap}>
                                                                    ...
                                                                    <View style={styles.TANDCInpItem}>
                                                                        <Text style={[styles.CustHeadText, { width: '20%' }]}>Name</Text>
                                                                        <View style={styles.TANDCInpCont}>
                                                                            <TextInput
                                                                                style={styles.NewInputStyle}
                                                                                ref={cashCustNameRef}
                                                                    ...
                                                                    <View style={styles.TANDCInpItem}>
                                                                        <Text style={[styles.CustHeadText, { width: '20%' }]}>Address</Text>
                                                                        <View style={styles.TANDCInpCont}>
                                                                            <TextInput
                                                                                style={styles.NewInputStyle}
                                                                    ...
                                                                    <View style={styles.TANDCInpItem}>
                                                                        <Text style={[styles.CustHeadText, { width: '20%' }]}>Phone Number</Text>
                                                                        <View style={styles.TANDCInpCont}>
                                                                            <TextInput
                                                                                style={styles.NewInputStyle}
                                                                    ...
                                                                    <View style={styles.TANDCInpItem}>
                                                                        <Text style={[styles.CustHeadText, { width: '20%' }]}>Remark</Text>
                                                                        <View style={styles.TANDCInpCont}>
                                                                            <TextInput
                                                                                style={styles.NewInputStyle}
                                                                                // placeholder='Enter order remark'
...

                            {
                                ...
                                <>
                                    <View style={styles.StockItemBox}>
                                        <View style={styles.StockItemWrap}>
                                            ...
                                            <View style={{
                                                ...
                                                <View style={[styles.TANDCInpCont, { width: '95%', marginTop: 12 }]}>
                                                    <TextInput
                                                        style={styles.NewInputStyle}
                                                        ref={searchItemInpRef}
                                                        placeholder='Search Item'
...

                                            {
                                                ...
                                                <>
                                                    ...
                                                    <ScrollView nestedScrollEnabled={true} contentContainerStyle={[styles.CheckStockListView]} keyboardShouldPersistTaps="handled">

                                                        {
                                                            stockData && stockData.length > 0 && stockData.map((item, index) => (
                                                                <TouchableOpacity style={[styles.StockListItem, { borderColor: 'grey', borderWidth: 0.5 }]} key={index}
                                                                    onPress={() => {
                                                                    ...
                                                                    <View style={styles.CustomerListCont}>
                                                                        ...
                                                                        <View style={styles.CustomerImgWrap}>
                                                                            <Image style={styles.CustomerImage} source={require('../images/openBoxW.png')} />
                                                                        </View>
                                            ...
                                            <View style={styles.AddtoCartInps}>
                                                ...
                                                <View style={[styles.TANDCInpItem, { width: '75%' }]}>
                                                    <Text style={[styles.CustHeadText, { width: '20%' }]}>Unit</Text>
                                                ...
                                                <View style={[styles.TANDCInpItem, { width: '75%' }]}>
                                                    <Text style={[styles.CustHeadText, { width: '20%' }]}>Qty</Text>
                                                ...
                                                <View style={[styles.TANDCInpItem, { width: '75%' }]}>
                                                    ...
                                                    <View style={[styles.TANDCInpCont, { marginTop: 0, width: '60%' }]}>
                                                        ...
                                                        <TextInput
                                                            ...
                                                            value={(cmpcode?.trim().toUpperCase() == "SUPERLAND" ||
                                                                cmpcode?.trim().toUpperCase() == "SALESDOODDEMO" ||
                                                                cmpcode?.trim().toUpperCase() == "POPULAR" || cmpcode?.trim().toUpperCase() != "AUTOMAX" ||
                                                                selectedStock?.Code == "CUSTOM-CODE") ? unitPrice : formatPrice3Decimal(unitPrice)}

                                                            editable={(cmpcode?.trim().toUpperCase() == "SUPERLAND" ||
                                                                cmpcode?.trim().toUpperCase() == "SALESDOODDEMO" ||
                                                                cmpcode?.trim().toUpperCase() == "POPULAR" || cmpcode?.trim().toUpperCase() != "AUTOMAX" ||
                                                                selectedStock?.Code == "CUSTOM-CODE") ? true : false}
                                                        />
                                                ...
                                                <View style={[styles.TANDCInpItem, { width: '75%' }]}>
                                                    <Text style={[styles.CustHeadText, { width: '20%' }]}>Total</Text>
                                                    <View style={[styles.AddItemInpCont, { marginTop: 0 }]}>
                                                        {/* <Text style={styles.AddCartInpLabel}>Unit</Text> */}
                                                        <View style={[{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', height: 35 }]}>
                                                            <Text style={[{ marginLeft: 4, color: '#aaa', width: '100%' }]}>{(quantity * unitPrice).toFixed(3)}</Text>
                                                        </View>
                                                    </View>
                                                </View>
                                    ...
                                    <View style={{
                                        ...
                                        <TouchableOpacity style={[styles.NextButton, { width: '30%' }]} onPress={() => {

                                            setShowCartPanel(false)

                                        }}>
                                            <Text style={[styles.CustHeadText, { color: 'white', textAlign: 'center' }]}>Back</Text>
                                        </TouchableOpacity>
                                        <TouchableOpacity style={[styles.AddCartButton, { width: '30%' }]} onPress={() => SaveItem()}>
                                            <Text style={[styles.CustHeadText, { color: 'white', textAlign: 'center' }]}>Add to Cart</Text>
                                        </TouchableOpacity>
                                    </View>
...

            {
                ...
                <View style={styles.UnitModalWrapper}>
                    <View style={styles.UnitModal}>
                        ...
                        <ScrollView nestedScrollEnabled={true} contentContainerStyle={styles.UnitScrollView}>
                            {
                                dummyUnits && dummyUnits.map((item, index) => (
                                    <TouchableOpacity style={styles.UntiItems} onPress={() => handleUnitClick(item.unit)}>
...

            {
                ...
                <View style={styles.UnitModalWrapper}>
                    <View style={styles.UnitModal}>
                        ...
                        <Text style={{ margin: 4, color: "#000000" }}>Checking Customer Due Days...Please wait</Text>
                        <ActivityIndicator />
                    </View>
                </View>
...

            {
                ...
                <View style={styles.UnitModalWrapper}>
                    <View style={styles.UnitModal}>
                        ...
                        <Text style={{ margin: 4, color: "#000000" }}>Checking if item is in Price Management...Please wait</Text>
                        <ActivityIndicator />
                    </View>
                </View>
