...
const SalesOrder = ({route}) => {
  ...
  const handleRemoveItem = async itemcode => {
    ...
    const filteredItems = savedItemData.filter(
      item => (item.Code || item.CODE || item.code) !== itemcode,
    );
  ...
  const EditItem = item => {
    setShowSelectedStockPop(false);
    handleEditRemoveItem(item.Code || item.CODE);
  ...
  const searchEditCustomer = async value => {
    ...
    console.log(
      'searchEditCustomer operation type',
      value,
      selectedRadio?.trim()?.toUpperCase(),
    );
    try {
      await axios
        .get(
      ...
      if (cmpcode?.trim().toUpperCase() !== 'AUTOMAX') {
        if (this_item_is_foc.current) {
          ...
          if (cmpcode?.trim().toUpperCase() == 'SOCA') {
            setUnitPrice(
              selectedStock['Credit Price']
                ? selectedStock['Credit Price']?.toString()
                : '',
            );
          } else {
            setUnitPrice(
              selectedStock['price'] ? selectedStock['price']?.toString() : '',
            );
          }
  ...
  const SaveItem = async () => {
    ...
    const itemExists = savedItemDataArray.some(itemInArray => {
      const existingCode =
        itemInArray.CODE || itemInArray.Code || itemInArray.code;
      const selectedCode =
        selectedStock.CODE || selectedStock.Code || selectedStock.code;
    ...
    if (
      ...
      if (
        ...
        console.log('All Saved Item', [...savedItemData, newItem]);
        setQuantity('');
        setUnitPrice('');
        setSelectedStock(null);
        // showSaveItemSuccess()
  ...
  useEffect(() => {
    ...
    if (
      ...
      const avaiBal = Number(selectedCustomer.Avai_Bal);
      const creditLimit = Number(selectedCustomer.Credit_Limit);
      const dueDays = Number(selectedCustomer.duedays);
  ...
  }, [selectedRadio, selectedCustomer]);
  ...
  const checkIfitemStockisZero = async item => {
    ...
    if (resultOfPriceManagement) {
      ...
      // // Check if the item already exists in the list
      const itemExists = savedItemDataArray.some(itemFilter => {
        // Get the code from the existing item (check all possible casings)
        const existingCode =
          itemFilter.Code || itemFilter.CODE || itemFilter.code;
        // Get the code from the current item being checked
        const currentCode = item.Code || item.CODE || item.code;
