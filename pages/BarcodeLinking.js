...
import { View, Text, ScrollView, StyleSheet, TextInput, FlatList, Dimensions, Touchable, TouchableOpacity, ActivityIndicator, Switch } from "react-native";
...
function BarcodeLinking({ navigation }) {

    const [showLoadingDetailsOfItem, setShowLoadingDetailsOfItem] = useState(false)
    const [currentSearchingTextToLink, setCurrentSearchingTextToLink] = useState("")
    const [currentSearchingText, setCurrentSearchingText] = useState("")
    const [selectedItemForBarcodeLinking, setSelectedItemForBarcodeLinking] = useState("")

    const SCREEN_WIDTH = Dimensions.get('window').width;


    const [askShowBarCodeLinkComponent, setAskShowBarCodeLinkComponent] = useState(false)
    const [showBarCodeLinkComponent, setShowBarCodeLinkComponent] = useState(false)
    ...
    const [stockOfItemInThisLocation, setStockOfItemInThisLocation] = useState(null)
    ...
    const [itemCameFromBarcodeReading, setItemCameFromBarcodeReading] = useState(false);
    ...
    const [showLocationSelectionPopup, setShowLocationSelectionPopup] = useState(false)
    ...
    const [arrayOfSearchResultsLinkBarcode, setArrayOfSearchResultsLinkBarcode] = useState(null);
    ...
    const callApi = (searchingText) => {
        ...
        console.log("specialCharacterRemovedItemCode ", specialCharacterRemovedItemCode)
    ...
    const getItemQuantityInThatLocation = (itemCode) => {
        ...
        axios.get(`${appUrl}Loc_Qty/locqty/${specialCharacterRemovedItemCode}/${deptNo}`).then((res) => {
    ...
    const getItemDetails = (itemCode) => {
        ...
        setShowLoadingDetailsOfItem(true)
        var specialCharacterRemovedItemCode = encodeURIComponent(itemCode);
        ...
        axios.get(apiUrl).then((res) => {
            ...
            setShowLoadingDetailsOfItem(false)
            ...
            if (res.data != null) {
                ...
                if (res.data.length > 0) {
                    ...
                    let ObjectAfterGettingDetails = {
                        ...
                        bin_location: objectOfItem.Bin != null ? objectOfItem.Bin.trim() : "",
                        picked_quantity: last4DigitQuantityRef.current != null ? last4DigitQuantityRef.current : "",
                        mode_of_entry: toggleForEntryType ? "R" : "N",
                    }
    ...
    const callItemTemporaryTableList = (itemCodeTemporaryList, locationCodeTemporaryList) => {


        setIsCallingTemporaryList(true)

        var specialCharacterRemovedItemCode = encodeURIComponent(itemCodeTemporaryList);

        let apiUrl = `${appUrl}PhysicalStk/${cmpCode}/${specialCharacterRemovedItemCode}/${currentChosenLocation?.trim()}`

        console.log("apiUrl for retrieving list again ---> ", apiUrl, itemCodeTemporaryList)
        axios.get(apiUrl).then((res) => {
    ...
    const checkForAnyMissingInputBIN = () => {

        // if only bin update is false then only show the error to input quantity
        if (!toggleForBinUpdate) {
            if (workListArray[0].bin_location != "") {

                return true;
            } else {
                console.log(" bin not entered ")
                setInputError("please enter BIN")
                return false;
            }
        } else {
            return true
    ...
    const sendDataToAPI = () => {
        ...
        axios.post(apiUrl, objectToSendToAPI).then((res) => {
            ...
            if (res.data.message == "Insert/Update successful") {
                ...
                callItemTemporaryTableList(objectToSendToAPI.code, objectToSendToAPI.loc)
    ...
    const sendBinDataToAPI = () => {

        if (!checkForAnyMissingInputBIN()) {
            return
        }
        ...
        const objectToSendToAPI = {
            ...
            idesc: workListArray[0].item_description,
            loc: currentChosenLocation?.trim(),
            sysqty: 0, // get stock from another api
            phyqty: 0,
            user: loginUsername,
            bin: workListArray[0].bin_location
        }
        ...
        //let apiUrl = `${appUrl}PhysicalStk/InsertPhyQty?compcode=${cmpCode}`
        let apiUrl = `${appUrl}PhysicalStk/UpdateBin?compcode=${cmpCode}`
    ...
    useEffect(() => {
        ...
        if (isAutoSaveChecked && itemCameFromBarcodeReading && workListArray.length > 0) {
            sendDataToAPI()
        } else {
    ...
    return (
        <View style={{ height: "100%", backgroundColor: "#ffffff", paddingBottom: 50 }}>

            <View style={{ flexDirection: "row", justifyContent: "space-between", backgroundColor: "#ffffff", marginTop: 20, paddingRight: 10 }}>
                <Text style={{ fontSize: 20, color: "#000000", backgroundColor: "#ffffff", paddingHorizontal: 20, }}>Barcode Linking</Text>
                {/* <TouchableOpacity style={{ width: 100, backgroundColor: "#000000", padding: 10, flexDirection: "row", justifyContent: "center" }} onPress={() => { itemSearchInputRef.current.focus(); setShowSettingsPanel(prev => !prev) }}><Text style={{ fontSize: 16, color: "#ffffff" }}>Settings</Text></TouchableOpacity> */}
            </View>




            <View style={{ backgroundColor: "#ffffff", flexDirection: "row", paddingLeft: 20, paddingVertical:10 }}>
                <Text style={{fontSize:16, color: "#000000" }}>Selected Location</Text>
                <Text style={{ fontSize:16, marginLeft: 10, color: "#000000" }}>{currentChosenLocation}</Text>
            </View>

            {
                currentSearchingText && askShowBarCodeLinkComponent &&
                <View style={{ padding: 10, backgroundColor: "#f7f7f7", margin: 10 }}>
                    <Text style={{fontSize:16, color:"#000000"}}>Item Not Found, Do you want to link</Text>
                    <View style={{ flexDirection: "row", justifyContent: "flex-end" }}>
                        <TouchableOpacity onPress={() => { setAskShowBarCodeLinkComponent(false) }} style={styles.cancelButton}><Text style={{ color: "#000000" }}>No</Text></TouchableOpacity>
                        <TouchableOpacity onPress={() => { { setAskShowBarCodeLinkComponent(false); setShowBarCodeLinkComponent(true) } }} style={[styles.saveButton, { marginLeft: 10 }]}><Text style={{ color: "#000000" }}>Yes</Text></TouchableOpacity>
                    </View>
                </View>
            }
...

            {
                showBarCodeLinkComponent ?
                    ...
                    <View style={{ padding: 20, borderWidth: 1, borderRadius: 5, marginHorizontal: 10, marginTop: 10 }}>
                        <View style={{ flexDirection: "row" }}>
                            <Text style={{ padding: 4, fontSize: 14, color: "#000000" }}>Barcode</Text>
                            <Text style={{ backgroundColor: "#d7d7d7", color: "#000000", padding: 4, fontSize: 16 }}>{currentSearchingText}</Text>
                        </View>

                        <View style={{ flexDirection: "row", marginTop: 10 }}>
                            <Text style={{ padding: 4, fontSize: 14, color: "#000000" }}>Selected Item To Link</Text>
                            <Text style={{ backgroundColor: "#d7d7d7", color: "#000000", padding: 4, fontSize: 16 }}>{selectedItemForBarcodeLinking}</Text>
                        </View>


                        <TextInput placeholder="Search item to link" style={styles.binInput} onChangeText={(text) => callApiForBarcodeLinking(text)}></TextInput>

                        {
                            arrayOfSearchResultsLinkBarcode?.length > 0 && currentSearchingTextToLink?.length > 0 &&
                            ...
                            <FlatList
                                ...
                                ListHeaderComponent={() => (
                                    <View style={{ flexDirection: "row", padding: 10, backgroundColor: "#000000" }}>
                                        <Text style={{ width: "50%", textAlign: "center", color: "#ffffff" }}>Code</Text>
                                        <Text style={{ width: "50%", textAlign: "center", color: "#ffffff" }}>Description</Text>
                                    </View>
                                )}
                                renderItem={({ item, index }) => (
                                    <TouchableOpacity onPress={() => clickedItemFromSearchListBarcodeLinking(item, index)} style={{ backgroundColor: "#f7f7f7", borderWidth: 1, borderColor: "#a7a7a7", padding: 10, flexDirection: "row" }}>


                                        <Text style={{ width: "50%", textAlign: "center", color: "#000000" }}>{item.Code}</Text>
                                        <Text style={{ width: "50%", textAlign: "center", color: "#000000" }}>{item.Description}</Text>
                        ...
                        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                            <TouchableOpacity onPress={() => setShowBarCodeLinkComponent(false)} style={styles.cancelButton}>
                                <Text style={styles.cancelButtonText}>Cancel</Text>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={sendDataToAPILinkBarCode} style={styles.saveButton}>
                    ...
                    <ScrollView style={[styles.container]} nestedScrollEnabled={true}>
                        ...
                        {showSettingsPanel && (
                            <View style={styles.settingsPanel}>
                                ...
                                >
                                    <View style={[styles.checkbox, isAutoSaveChecked && styles.checkboxChecked]} />
                        ...
                        {showHideSearchListBox && (
                            ...
                            <FlatList
                                ...
                                renderItem={({ item, index }) => (
                                    <TouchableOpacity onPress={() => clickedItemFromSearchList(item, index)} style={{ backgroundColor: "#f7f7f7", borderWidth: 1, borderColor: "#a7a7a7", padding: 10, flexDirection: "row" }}>
                        ...
                        {
                            showLoadingDetailsOfItem && <Text style={{color:"#000000", fontSize:18,padding:10}}>Checking Please Wait...</Text>
                        }
                        {/* Work List Entry Card */}
                        {workListArray.length > 0 && (
                            <View style={styles.entryCard}>
                                <Text style={styles.cardLabel}>Item Code</Text>
                                <Text style={{ color: "#000000" }}>{workListArray[0].item_code}</Text>

                                <Text style={styles.cardLabel}>Description</Text>
                                <Text style={{ color: "#000000" }}>{workListArray[0].item_description}</Text>
                                ...
                                {inputError.length > 0 && <Text style={styles.errorText}>{inputError}</Text>}

                                <Text style={styles.cardLabel}>Bin</Text>
                                <TextInput
                                    ref={entryCardBinInputRef}
                                    style={styles.binInput}
                                    value={isDefaultBinChecked ? defaultBinName : workListArray[0].bin_location}
                                    editable={!isDefaultBinChecked}
                                    onChangeText={(text) => !isDefaultBinChecked && handleBinInput(text, 0)}
                                />

                                <TouchableOpacity onPress={removeItemFromRow} style={styles.removeButton}>
                                    <Text style={styles.removeButtonText}>🗑 CLEAR</Text>
                                </TouchableOpacity>

                                <TouchableOpacity onPress={sendBinDataToAPI} style={styles.saveButton}>
                        ...
                        {/* {temporaryTableItemList?.length > 0 && (
                            <View style={styles.summaryContainer}>
                                <Text style={{ fontSize: 20, fontWeight: "400", color: "#000000" }}>Previous Entries</Text>
                                <Text style={{ color: "#000000" }}>Code: {temporaryTableItemList[0].code}</Text>
                                <Text style={{ color: "#000000" }}>Description: {temporaryTableItemList[0].description}</Text>
                                <Text style={{ color: "#000000" }}>
                                    Total Stock Counted:{" "}
                                    {temporaryTableItemList.reduce((acc, curr) => acc + curr.phyQty, 0)}
                                </Text>
                            </View>
                        )} */}
                        ...
                        {/* {
                            ...
                            <ScrollView horizontal style={{ width: SCREEN_WIDTH, paddingBottom: 50 }} >
                                <FlatList
                                    style={{ backgroundColor: "#a7a7a7", width: SCREEN_WIDTH, marginTop: 10 }}
                                    data={temporaryTableItemList}
                                    keyExtractor={(item, index) => index.toString()}
                                    ListHeaderComponent={() => (
                                        <View style={{ flexDirection: "row", padding: 10, backgroundColor: "#000000" }}>

                                            <Text style={{ width: "30%", textAlign: "center", color: "#ffffff" }}>PhyQty</Text>
                                            <Text style={{ width: "30%", textAlign: "center", color: "#ffffff" }}>Date Time</Text>
                                            <Text style={{ width: "40%", textAlign: "center", color: "#ffffff" }}>User</Text>
                                        </View>
                                    )}
                                    renderItem={({ item, index }) => (
                                        <TouchableOpacity onPress={() => clickedItemFromSearchList(item, index)} style={{ backgroundColor: "#f7f7f7", borderWidth: 1, borderColor: "#a7a7a7", padding: 10, flexDirection: "row" }}>
                                            ...
                                            <View style={{ width: "30%", flexDirection: "column" }}>
                                                <Text style={{ textAlign: "center", color: "#000000", fontWeight: "700" }}>{item.phyQty}</Text>
                                                <Text style={{ textAlign: "center", color: "#000000" }}> {item.bin}</Text>
                                            </View>

                                            <Text style={{ width: "30%", textAlign: "center", color: "#000000" }}>{getReadableDate(item.dtime) + " " + getReadableTime(item.dtime)}</Text>
                                            <Text style={{ width: "40%", textAlign: "center", color: "#000000" }}>{item.user}</Text>
...

            {
                ...
                <View style={{ position: "absolute", backgroundColor: "#00000080", padding: 30, height: "100%", top: 0 }}>

                    <View style={{ width: "100%", height: "80%", padding: 20, backgroundColor: "#f7f7f7" }}>

                        <View style={{ flexDirection: "row", backgroundColor: "#ffffff", }}>
                            <Text style={{ fontSize: 20, fontWeight: "600", color: "#000000", padding: 10 }}>Please Select Location</Text>
                        </View>
                        <View style={{ flexDirection: "row", backgroundColor: "#ffffff", paddingTop: 20 }}>
                            <Text style={{ width: "50%", padding: 10, color: "#000000" }}>Code</Text>
                            <Text style={{ width: "50%", padding: 10, color: "#000000" }}>Description</Text>
                        </View>
                        <ScrollView >
                            {locationList?.map((item) => {
                                return (
                                    <TouchableOpacity onPress={() => { setnoLocationIsSet(prev => !prev); setCurrentChosenLocation(item.loc_code) }} style={{ backgroundColor: "#f7f7f7", borderWidth: 1, borderColor: "#d7d7d7", flexDirection: "row" }}><Text style={{ width: "50%", padding: 10, color: "#000000" }}>{item.loc_code}</Text>
                                        <Text style={{ width: "50%", color: "#000000", padding: 10 }}> {item.DESC}</Text></TouchableOpacity>
                                )
                            })}
                        </ScrollView>
...
const styles = StyleSheet.create({
    ...
    heading: { fontWeight: 'bold', fontSize: 18, color: "#000000" },
    ...
    searchInput: { borderWidth: 1, borderColor: '#ccc', padding: 10, borderRadius: 5, color: "#000000" },
    searchResultsContainer: { marginTop: 10 },
    searchItemRow: { paddingVertical: 8, borderBottomWidth: 1, borderColor: '#eee' },
    searchItemText: { fontWeight: 'bold' },
    entryCard: { padding: 15, backgroundColor: '#f9f9f9', borderRadius: 8, marginTop: 20 },
    cardLabel: { fontWeight: 'bold', marginTop: 10, color: "#000000" },
    quantityInput: { borderWidth: 1, padding: 8, borderRadius: 5, marginTop: 5, color: "#000000" },
    binInput: { borderWidth: 1, padding: 8, borderRadius: 5, marginTop: 5, color: "#000000" },
    errorText: { color: 'red', marginTop: 5 },
    removeButton: { alignSelf: 'flex-end', marginTop: 10 },
    removeButtonText: { color: 'red' , fontSize:16},
    saveButton: {
