...
import CheckListPop from '../popups/CheckListPop'
...
const CheckListNew = () => {
    ...
    const fetchPickList = async () => {
        ...
        try {
            const response = await axios.get(`${appUrl}Sales_Order/${cmpCode}/CHECKING/-/${deptno}/-`)
            console.log(`${appUrl}Sales_Order/${cmpCode}/CHECKING/-/${deptno}/-`)
    ...
    const checkIfAlreadyAcceptItem = async (sono) => {
        ...
        try {
            ...
            console.log('acceptItemUrl', `${appUrl}Sales_Order/automax/CHECKING_DUPLICATE/-/${deptno}/1233`)

            const response = await axios.get(`${appUrl}Sales_Order/automax/CHECKING_DUPLICATE/-/${deptno}/${sono}`);

            if (response.data?.length > 0) {

                console.log(response.data)

                if (response.data[0].STATUS == "PICKING COMPLETED") {
                    acceptItem(sono)
                } else {
    ...
    return (
        <>
            <View style={styles.HomeWrap}>
                ...
                <HeaderUiNew name={'Checking List'} />
...

                    {
                        ...
                        <>
                            <View style={styles.ActionTabBox}>
                                ...
                                >
                                    <Text style={styles.ButtonText}>My Check List</Text>
                                    <Text style={[styles.ButtonText, { marginTop: 12 }]}>
